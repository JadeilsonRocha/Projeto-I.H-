...
event btn_envio_state(void) {
  ...
  if(WiFi.status() == WL_CONNECTED){//Verificar se há conexão wifi
    if(client.connect("ESP8266Client", "A1E-KqqzbpU37bzMoWVPnwi8Z9kRTMj2Fn", "")){//Verificar se há conexão com mqtt
      if(enviar_dados()){
...
event conexao_wifi_state(void) {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.print(".");
    Serial.println("Continuo no estado conexão WIFI!");
    delay(3000);
    return evento_wifi;
  }
}
